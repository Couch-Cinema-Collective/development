...
/** Guild home: roster and, for the commissioner, the invite link. */
    ...
    <main className="mx-auto max-w-4xl px-6 py-12">
      <header className="border-b border-rule pb-8">
        <p className="label-eyebrow">
          {guild.role === "commissioner" ? "Your guild · Commissioner" : "Your guild"}
        </p>
      ...
      {guild.role === "commissioner" && (
        <section className="mt-10 border border-rule bg-paper-raised p-6">
          <h2 className="label-eyebrow">Invite your members</h2>
          <div className="mt-4 flex flex-wrap items-center gap-4">
            <code className="min-w-0 flex-1 break-all text-sm">{inviteUrl}</code>
      ...
      <section className="mt-12">
        ...
        {current ? (
            ...
            <p className="mt-3 text-sm text-ink-soft">
              ...
              {current.state === "NOMINATING" && current.nominationDeadline && (
                <>
                  {" "}
                  · nominations lock{" "}
                  {new Date(current.nominationDeadline).toLocaleDateString(
                    "en-US",
          ...
          <p className="mt-4 max-w-xl text-sm leading-relaxed text-ink-soft">
            No season running yet — your commissioner opens the next one.
          </p>
        )}

        {seasons.some((s) => s.state === "PUBLISHED" || s.state === "ARCHIVED") && (
          <ul className="mt-6 grid gap-px border border-rule bg-rule">
            {seasons
              .filter((s) => s.state === "PUBLISHED" || s.state === "ARCHIVED")
              .map((s) => (
                <li
                  key={s.id}
                  className="flex items-baseline justify-between gap-6 bg-paper-raised px-6 py-4"
